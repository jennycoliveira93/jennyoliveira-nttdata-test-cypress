...
describe('Funcionalidade: Tela de Login', () => {
    ...
    context('Dado que eu estou na tela de Login do ServeRest', () => {
        ...
        context('Cenário 1: Tentar logar com dados inválidos', () => {
            const exemplos = [
                { email: 'emailInvalidoVazio', senha: 'senhaValida', mensagem: 'Email é obrigatório' },
                { email: 'emailValido', senha: 'senhaInvalidaVazia', mensagem: 'Password é obrigatório' },
        ...
        context('Cenário 2: Logar com sucesso na aplicação', () => {
            context('Quando eu preencher os campos email e senha com valores válidos e existentes', () => {
                beforeEach(() => {
                    cy.fixture('usuario').then((usuarioFixture) => {
                        cy.realizarLogin(usuarioFixture.adminValido.emailValido, usuarioFixture.adminValido.senhaValida)
                    })
                })
